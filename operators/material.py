...
_active_lamp_transitions_global = {}
...
class LampSwitcher:
    class SCS_TOOLS_OT_SwitchLampmask(bpy.types.Operator):
        ...
        bl_description = "Show/Hide specific areas of lamp mask texture with smooth transition."

        lamp_type: StringProperty(default="", options={'HIDDEN'})
        transition_duration: FloatProperty(default=6.0, min=0.0)

        def _clear_existing_timer(self):
            """Membatalkan timer yang ada untuk lamp_type ini."""
            global _active_lamp_transitions_global
            if self.lamp_type in _active_lamp_transitions_global:
                active_transition = _active_lamp_transitions_global[self.lamp_type]
                if active_transition.get('timer_active', False) and active_transition.get('timer_func'):
                    try:
                        bpy.app.timers.unregister(active_transition['timer_func'])
                    except ValueError:
                        pass # Timer mungkin sudah tidak ada
                del _active_lamp_transitions_global[self.lamp_type]

        def _start_transition(self, context, input_socket_ref, current_value, target_value):
            """Memulai transisi baru."""
            global _active_lamp_transitions_global

            transition_data = {
                "lamp_type_name": self.lamp_type,
                "input_socket_ref": input_socket_ref,
                "start_value": current_value,
                "target_value": target_value,
                "duration_frames": self.transition_duration,
                "start_time_monotonic": time.monotonic(),
                "timer_func": None,
                "timer_active": True
            }

            def smooth_transition_step():
                global _active_lamp_transitions_global
                # Ambil data yang relevan dari dict global menggunakan nama lamp_type dari closure
                active_data = _active_lamp_transitions_global.get(transition_data["lamp_type_name"])

                if not active_data or not active_data.get("timer_active", False):
                    return None

                elapsed_time = time.monotonic() - active_data["start_time_monotonic"]
                scene_fps = context.scene.render.fps if context.scene.render.fps > 0 else 24.0
                duration_seconds = active_data["duration_frames"] / scene_fps

                progress = min(elapsed_time / duration_seconds, 1.0) if duration_seconds > 0.0001 else 1.0
                
                interpolated_val = active_data["start_value"] + \
                                   (active_data["target_value"] - active_data["start_value"]) * progress
                active_data["input_socket_ref"].default_value = interpolated_val

                if progress >= 1.0:
                    active_data["input_socket_ref"].default_value = active_data["target_value"]
                    print(f"'{active_data['lamp_type_name']}' transition finished to {'ON' if active_data['target_value'] > 0.5 else 'OFF'}.")
                    if active_data["lamp_type_name"] in _active_lamp_transitions_global:
                        _active_lamp_transitions_global[active_data["lamp_type_name"]]['timer_active'] = False
                    return None
                return 1.0 / scene_fps

            transition_data["timer_func"] = smooth_transition_step
            _active_lamp_transitions_global[self.lamp_type] = transition_data
            bpy.app.timers.register(smooth_transition_step, first_interval=0.001)
            
            report_message = f"'{self.lamp_type}' transitioning to {'ON' if target_value > 0.5 else 'OFF'} over {self.transition_duration} frames."
        ...
        def execute(self, context):
            ...
            if LAMPMASK_MIX_G not in bpy.data.node_groups:
                self.report({"ERROR"}, f"{LAMPMASK_MIX_G} not found.")
                return {'CANCELLED'}

            lampmask_nodes = bpy.data.node_groups[LAMPMASK_MIX_G].nodes
            if self.lamp_type not in lampmask_nodes:
                self.report({"ERROR"}, f"Node '{self.lamp_type}' not found in {LAMPMASK_MIX_G}.")
                return {'CANCELLED'}

            input_socket = lampmask_nodes[self.lamp_type].inputs[0]
            self._clear_existing_timer() # Batalkan timer lama dulu

            current_val = float(input_socket.default_value)
            target_val = 1.0 if current_val < 0.5 else 0.0

            if self.transition_duration <= 0:
                input_socket.default_value = target_val
                self.report({"INFO"}, f"'{self.lamp_type}' set to {'ON' if target_val > 0.5 else 'OFF'}.")
            else:
                self._start_transition(context, input_socket, current_val, target_val)
